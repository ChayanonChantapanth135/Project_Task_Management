...
const Profile = () => {
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        await signIn({
          token: currentToken,
          expiresInSeconds: expiresAt ? Math.round((Number(expiresAt) - Date.now()) / 1000) : 2400,
          user: {
            id: updatedUser.id,
  ...
  if (loading) {
    return (
      <div className="d-flex justify-content-center align-items-center bg-[#153648] text-white" style={{ minHeight: "100vh" }}>
    ...
    <div ref={pageRef} className="min-h-screen flex flex-col bg-[#153648] text-slate-100 font-sans selection:bg-teal-500 selection:text-white relative overflow-hidden">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-8 relative z-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-xs text-slate-400 mt-1">จัดการข้อมูลส่วนตัวและความปลอดภัยบัญชีของคุณ</p>
        </div>
        ...
        {errorMsg && (
          <div className="mb-6 w-full py-3.5 px-5 rounded-2xl bg-[#521c22] text-rose-300 text-sm font-semibold flex items-center gap-3 shadow-xl border-0 animate-fade-in-down">
            <span className="w-4 h-4 rounded bg-rose-500 text-slate-950 flex items-center justify-center text-[10px] font-black">!</span>
            <span>{errorMsg}</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="glass-panel rounded-3xl p-6 shadow-2xl flex flex-col items-center text-center">
              ...
              <div className="relative group mb-4">
                ...
                >
                  {avatarPreview ? (
                    ...
                    <span className="text-4xl font-extrabold">{username ? username[0]?.toUpperCase() : "U"}</span>
                  )}
                </div>
              ...
              <div className="flex flex-wrap gap-2 justify-center mb-6">
                <span className="px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                  🛡️ {user?.role}
                </span>
                <span className="px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                  ✨ {user?.status === "active" ? "Active" : "Suspended"}
                </span>
              </div>

              <div className="w-full text-left bg-slate-800/40 rounded-2xl p-4 text-xs text-slate-300 space-y-2 border border-slate-700/30">
          ...
          <div className="lg:col-span-2 space-y-6">
            <form onSubmit={handleSaveProfile} className="space-y-6">
              ...
              <div className="glass-panel rounded-3xl p-6 shadow-2xl space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-slate-400 font-semibold">ชื่อผู้ใช้ (Username)</label>
                    <input
                      type="text"
                  ...
                  {/* Phone */}
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-slate-400 font-semibold">เบอร์โทรศัพท์ (Phone)</label>
                    <input
                      type="tel"
                      className="bg-slate-800/60 hover:bg-slate-800/80 focus:bg-slate-900/90 text-white rounded-2xl px-4 py-3 text-sm transition-all"
                      value={phone}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Email (Disabled) */}
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-slate-400 font-semibold opacity-70">อีเมล (Email - ไม่สามารถเปลี่ยนได้)</label>
                    <input
                      type="email"
              ...
              <div className="glass-panel rounded-3xl p-6 shadow-2xl space-y-4">
                <h5 className="text-base font-bold text-teal-400 flex items-center gap-2 mb-2">
                  <i className="bi bi-shield-lock"></i> เปลี่ยนรหัสผ่าน (หากต้องการเปลี่ยน)
                </h5>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* New Password */}
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-slate-400 font-semibold">รหัสผ่านใหม่ (New Password)</label>
                    <input
                      type="password"
                      className="bg-slate-800/60 hover:bg-slate-800/80 focus:bg-slate-900/90 text-white rounded-2xl px-4 py-3 text-sm transition-all"
                      value={newPassword}
                  ...
                  {/* Confirm Password */}
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-slate-400 font-semibold">ยืนยันรหัสผ่านใหม่ (Confirm Password)</label>
                    <input
                      type="password"
                      className="bg-slate-800/60 hover:bg-slate-800/80 focus:bg-slate-900/90 text-white rounded-2xl px-4 py-3 text-sm transition-all"
